...
int h_low = 0, h_high = 0;
int s_low = 0, s_high = 0;
int v_low = 0, v_high = 0;
...
void CaptureMode(bool calib_capture_mode, string save_to_folder, int cam_left, int cam_right)
{
...
void stereoVision(int cam_left, int cam_right, bool use_webcams, string load_from_folder)
{
	...
	for (int i = 0; i < 1; i++)
	{
		...
		{
			image_left = imread(load_from_folder + "/DataLeft0.jpg");
			image_right = imread(load_from_folder + "/DataRight0.jpg");
		}
	...
	for (;;)
	{
		...
		{
			if (num_data_sample == 13)
				return;
			image_left = imread(load_from_folder + "/DataLeft" + to_string(num_data_sample) + ".jpg");
			image_right = imread(load_from_folder + "/DataRight" + to_string(num_data_sample) + ".jpg");
			num_data_sample++;
		}
		...
		minMaxLoc(imgDisparity16S, &minVal, &maxVal);
		imgDisparity16S.convertTo(imgDisparity8U, CV_8UC1, 255 / (maxVal - minVal));
		//Alternative for disparity image (better result; much slower)
		/*
		...
		*/
		...
		//Getting the area, perimeter and centers of the contours that are larger than a threshold area
		...
		for (int i = 0; i < contour_areas.size(); i++)
		{
			...
			for (int l = 0; l < contour_areas.size(); l++)
			{
				...
				//Isolated contours are considered imperfections produced in the disparity image
		...
		if (found_object)
		{
			...
			for (final_objects_iter = final_objects.begin(); final_objects_iter != final_objects.end(); final_objects_iter++)
			{
				...
				if (object_area > threshold_contour_area)
				{
					...
					rectangle(ROI, object_boundary, boundary_color, 2);
					if (is_cylindrical && !is_skin)
					{
						putText(ROI, "Cylindrical", Point(object_boundary.x, object_boundary.y + 10), 1, 1, Scalar(0, 255, 0));
					}
				}
			}
...
void main()
{
	...
	// The program is non menu driven for faster testing.
	// Setting the folder names is only required once.
	// The boolean options can be changed for different functionalities.

	bool use_webcams = true;                     //Set to false to use existing data. Must be true if 
												 //calibration data AND test data
												 //do not already exist.

	// **********Data collection options for non webcam testing mode***************************
	bool capture_new_data = false;                  //by default
	...
	int cam_left = 2;                            //Check from device manager
	int cam_right = 1;                           //Check form device manager

	string save_calib_data_to_folder = "";       //Change to folder where you want to store calibration data
	string load_calib_data_xml = "";             //Change to xml file containing complete path of all calibration images
	...
	if (capture_new_data)
	{
		if (capture_new_calib_data)
		{
			CaptureMode(true, save_calib_data_to_folder, cam_left, cam_right);
		}
		else
		{
			CaptureMode(false, save_test_data_to_folder, cam_left, cam_right);
		}
	}
	...
	if (run_calibration)
	{
		...
		bool ok = readStringList(load_calib_data_xml, image_list);
		if (!ok || image_list.empty())
		{
			cout << "can not open " << load_calib_data_xml << " or the string list is empty" << endl;
		}

		StereoCalib(image_list, boardSize, true, showRectified);
	}

	stereoVision(cam_left, cam_right, use_webcams, load_test_data_from_folder);
}